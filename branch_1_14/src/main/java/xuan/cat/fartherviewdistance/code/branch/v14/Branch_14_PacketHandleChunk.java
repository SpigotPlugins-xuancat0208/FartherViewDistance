...
public final class Branch_14_PacketHandleChunk {
    ...
    public Branch_14_PacketHandleChunk() {
        try {
            ...
            field_chunkBytes        = chunkClass.getDeclaredField("e");
            field_titleEntityNBT    = chunkClass.getDeclaredField("f");
            field_isFullChunk       = chunkClass.getDeclaredField("g");
    ...
    private int getExistDataSection(PacketDataSerializer dataSerializer, Chunk chunk, int existChunkSection, boolean isFullChunk) {
        ...
        if (isFullChunk) {
            ...
            for (BiomeBase biomeBase : biomeBases) {
                dataSerializer.writeInt(IRegistry.BIOME.a(biomeBase));
            }
        }
    ...
    private int getTotalByteLength(Chunk chunk, int existChunkSection, boolean isFullChunk) {
        ...
        if (isFullChunk) {
            byteLength += chunk.getBiomeIndex().length * 4;
        }
